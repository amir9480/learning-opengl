	...
	:Node()
{
	...
	mAlbedo = new Texture(mWidth, mHeight);
	glBindTexture(GL_TEXTURE_2D, mAlbedo->mTexture);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, mAlbedo->mTexture, 0);
...
Texture* Camera::getAlbedo() const
{
	return mAlbedo;
}
